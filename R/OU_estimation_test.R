...
#
# Function for computing the MLE estimator of theta, given an OU process into a
# fdata (fda.usc package) object as a sde: a fdata object with n = 1
#
...
#
# Function for predicting an OU as an ARH(1) process
#
...
#
# Given a stochastic process {X_n}_{n in Z}, this function, firstly, builds the
# functional model Y = rho(X) with Y = X_n and X = X_{n-1}, and secondly,
# computes the F-statistic between FLMFR (unrestricted) vs OU (that is, a
# particular ARH(1), with a specific operator rho, understood as a particular
# case of FLMFR),
# 
# H0: rho linear operator corresponding to the autocorr. op. of an OU as ARH(1)
...
F_stat_OU <- function(X_flmfr, X, est_method = "fpcr_l1s", thre_p = 0.99,
  ...
  RSS_OU <- mean(apply(pred_OU[["residuals"]][["data"]]^2, 1,
                       FUN = "integral1D", pred_OU[["residuals"]][["argvals"]]))
  ...
  RSS_FLMFR <- mean(apply(FLMFR_est[["residuals"]][["data"]]^2, 1,
                          FUN = "integral1D", X_flmfr$Y_fdata[["argvals"]]))
  
  # Computing F-statistic
  F_stat <- (RSS_OU - RSS_FLMFR) / RSS_FLMFR
  
  # Output
  return(list("F_stat" = F_stat, "RSS_FLMFR" = RSS_FLMFR, "RSS_OU" = RSS_OU,
              "pred_OU" = pred_OU, "FLMFR_est" = FLMFR_est))
}
...
#
# Generate functional stochastic processes
#
# OU <- r_stoch_proc(500, seq(0, 1, l = 101),
...
#
# Function for converting functional stochastic process into a FLMFR
# given by Y = X_n vs X = X_{n-p}
# 
# OU <- r_stoch_proc(500, seq(0, 1, l = 101),
#                    par.sde = list("alpha" = 0, "beta" = 0.5, "sigma" = 0.1),
#                    warm_up = -1, model = "OU")
...
#
# Function for testing if a stochastic process {X_n} is an ARH(1) by testing if
# X_n = rho(X_{n - 1}) + eps_n, being rho a linear operator (as a FLMFR)
#
...
ARHp_test <- function(X, p = 1, B = 1000, hyp_simp = FALSE, hyp_comp = TRUE,
  ...
  X_flmfr <- ARH_to_FLMFR(X[["fstoch_proc"]], p)
...
#
# Function for implementing a two-stages testing procedure:
#
# STAGE 1:
...
#     H1: X_n = rho(X_{n-1}) an alternative ARH(1) model
#
#
# Example generating the Ait-Sahalia model:
# 
...
test_gof_OU <- function(n, t = seq(0, 1, l = 71),
                        par.sde = list("alpha" = 0, "beta" = 0.5, "sigma" = 1),
                        mu = par.sde[["alpha"]] / par.sde[["beta"]], X0 = mu,
                        warm_up = 50, type = "CKLS", model = "OU", p = 1, B = 1000,
  ...
  testing_ARHp <- ARHp_test(X, p = p, B = B, hyp_simp = hyp_simp,
  ...
  sigma_est <- abs(optimize(mce, r = as.vector(X$stoch_proc$data), Delta = 1/(length(t) - 1), interval = c(0,2))$minimum)
  ...
  for (b in 1:B) {
    ...
    X_flmfr_ast <- ARH_to_FLMFR(X_ast[["fstoch_proc"]], p)
  ...
  result <-
    structure(list(statistic =
                     c("ARH(0)-stat" = testing_ARHp$testing_simple$statistic,
                       "ARH(1)-stat" = testing_ARHp$testing_comp$statistic,
                       "F-stat OU" = F_stat),
                   p.value = c("GOF ARH(0)" = testing_ARHp$testing_simple$p.value,
                               "GOF ARH(1)" = testing_ARHp$testing_comp$p.value,
                               "F-test OU" = p_value_F_test),
                   boot_statistics =
                     c("GOF ARH(0)" = testing_ARHp$testing_simple$boot_statistics,
                       "GOF ARH(1)" = testing_ARHp$testing_comp$boot_statistics,
                       "F-test" = F_stat_ast),
                   method = meth,
                   parameter = testing_ARHp$testing_comp$parameter,
                   p = testing_ARHp$testing_comp$p,
                   q = testing_ARHp$testing_comp$q,
                   fit_flm = testing_ARHp$testing_comp$fit_flm,
